...
<body class="fix-header fix-sidebar card-no-border">
    ...
    <div id="main-wrapper">
        ...
        <?php
          if(isset($_POST['id'])){
            $sql = "SELECT * FROM bookmark WHERE m_uname = '".$_SESSION['uname']."' AND document_id = '".$_POST['id']."'";
            $result = mysqli_query($link,$sql);
            if(mysqli_num_rows($result) > 0){
              while($status = mysqli_fetch_assoc($result)){
                if($status['b_status'] == 'no'){
                  $sql = "UPDATE bookmark SET b_status = 'yes' WHERE m_uname = '".$_SESSION['uname']."' AND document_id = '".$_POST['id']."'";
                  mysqli_query($link,$sql);
                }else{
                  $sql = "DELETE FROM bookmark WHERE m_uname = '".$_SESSION['uname']."' AND document_id = '".$_POST['id']."'";
                  mysqli_query($link,$sql);
                }
              }
            }else{
              $sql = "INSERT INTO bookmark(m_uname,document_id,b_status) VALUES('".$_SESSION['uname']."','".$_POST['id']."','yes')";
              mysqli_query($link,$sql);
            }
          }
        ?>
            ...
            <div class="container-fluid">
                ...
                <div class="row page-titles">
                    <div class="col-md-5 align-self-center">
                        <h3 class="text-themecolor">ติดดาว</h3>
                        <ol class="breadcrumb">
                    ...
                    <div class="col-12">
                        <div class="card card-body mailbox">
                            ...
                            <div id="sh_document" class="message-center ps ps--theme_default ps--active-y" data-ps-id="a045fe3c-cb6e-028e-3a70-8d6ff0d7f6bd">
                                ...
                                <?php
                                  if($status == 1){
                                    $sql = "SELECT * FROM document INNER JOIN member,bookmark WHERE member.m_uname = document.m_uname AND document.d_id = bookmark.document_id ORDER BY d_id DESC";
                                  }else{
                                    $sql = "SELECT * FROM document INNER JOIN member,send,bookmark WHERE send.s_document = document.d_id AND send.s_form = member.m_uname AND send.s_uname = '".$_SESSION['uname']."' AND document.d_id = bookmark.document_id ORDER BY d_id DESC";
                                  }
                                  $result = mysqli_query($link,$sql);
                                  if(mysqli_num_rows($result) > 0){
                                    ...
                                    echo '<p style="position:relative;top:50%;transform:transitionY(-50%);" class="text-center">ไม่มีรายการ</p>';
                                  }
                                ?>
                            </div>
                        </div>
